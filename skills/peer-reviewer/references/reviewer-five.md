...
Follow the shared Output rule in `SKILL.md`. In the summary, distinguish (1) what the paper does technically — brief and uncertain — from (2) what it claims for the domain — precise. Defer explicitly on technical content (one line) and spend most of your budget on domain relevance and implications — that's your contribution. Recommendation justified on domain relevance (note if your assessment is partial); Confidence High on domain aspects but Medium/Low on technical correctness (state which).
